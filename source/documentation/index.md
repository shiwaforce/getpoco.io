...
- Git, SVN support. Project catalog, multiple catalogues, multiple plans. Script support (before_script, **before_docker_script** for host-side commands per OS: windows, darwin, linux).
...
### Poco Repo
Poco Repo is a collection of Poco projects. Use `poco repo add`, `poco repo ls`, `poco repo pull`, `poco repo push` to manage repos. Example catalog entry:
```yaml
my-project-1:
  file: poco.yml
...
### Poco Catalog
Poco Catalog is a collection of Poco Repos. Add multiple repos from the same or different sources. Use `poco project add`, `poco project ls`, `poco catalog` to work with projects.
